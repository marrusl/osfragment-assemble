...
```
osfragment [--manifest osfragment.yaml] [--output Containerfile]
osfragment inspect <fragment-image-or-directory>
osfragment list [--manifest osfragment.yaml]
```
...
**Flags:**
- `--manifest <path>`: Manifest file (default: `./osfragment.yaml`)
...
```
$ osfragment inspect quay.io/marrusl2/fragments/tailscale:1.82.0
...
The `inspect` command retains local directory support (`osfragment inspect ./my-fragment/`) for examining fragments during development — this reads files directly with no prebuild infrastructure.
...
```dockerfile
# Generated by osfragment v0.1.0
# Manifest: osfragment.yaml
...
1. `osfragment` reads a manifest and generates a correct multi-stage Containerfile
