...
A fragment is an OCI image carrying three things. `fragment.toml` holds flat facts, on the model of an OCI annotation set: name, version, vendor, provides, conflicts, and the packages it needs in order to be itself. Every field describes the unit, none the machine it lands on. `tree/` is delivered payload: files copied into the image verbatim. `hooks/` is build input, not payload: the tool runs one executable, `hooks/entrypoint`, once, through a bind mount lasting only its build step; nothing of it persists in the image. The composing side is a few YAML lines: a base image and fragments, each optionally selecting packages or naming a mirror to rewrite its repo definitions against.
