...
`hook/` is build input, not payload. A fragment that carries hooks names one executable, `hook/entrypoint`, and that is the only thing the tool runs: once, through a bind mount that exists only for the duration of its build step, with everything else under `hook/` riding along as support material for it. The hook does its work, and nothing of it persists in the image.

`mount/` is build input that must not persist. Its subtree mirrors target paths the same way `tree/` does, but the files are bind-mounted onto the package install step and every hook step rather than copied into the image: material at `mount/etc/pki/entitlement/cert.pem` is readable at `/etc/pki/entitlement` while packages install and while hooks run, and the tool commits none of it. That guarantee belongs to the tool alone: hook code is fragment-supplied and runs as root, so a hook could still copy the mounted material into a layer, and a credential fragment, entitlement or otherwise, should pair only with hook fragments you trust. The case it exists for is package acquisition that has to authenticate: entitlement certificates, mirror client certificates, CA bundles for a TLS-intercepting proxy. Today that works through host-coupled engine magic, which only works on the right host, or through per-build secret plumbing, which is different for every builder and cannot be shipped by a second author. An artifact-coupled declared mount is pinnable, versionable, and composable like everything else here, and a fragment that must be pinned by digest is one whose trust material cannot be swapped by moving a tag.
